...
# Fechas del operativo — se leen de config para que cada municipio use las suyas
from config import INICIO_OPERATIVO as _INICIO_OP_STR
INICIO_OPERATIVO    = pd.Timestamp(_INICIO_OP_STR).date()
FECHA_BATCH_MANUAL  = INICIO_OPERATIVO   # sin batch manual en Atlixco: misma fecha = sin efecto
INICIO_LOGIN_BUBBLE = INICIO_OPERATIVO   # todos los registros usan Created By desde el inicio
...
def _transform(records: list[dict]) -> pd.DataFrame:
    ...
    # ── Fechas inicio/fin — defensivo: Atlixco no tiene campos explícitos ──────
    # Zacatlán: campos 'fecha_inicio'/'fecha_fin' en Bubble.
    # Atlixco:  usa 'fecha_creacion'/'fecha_modificacion' como fallback.
    for col, fallback in [("fecha_inicio", "fecha_creacion"),
                          ("fecha_fin",    "fecha_modificacion")]:
        if col in df.columns:
            df[col] = pd.to_datetime(df[col], utc=True, errors="coerce")
        elif fallback in df.columns:
            df[col] = pd.to_datetime(df[fallback], utc=True, errors="coerce")
        else:
            df[col] = pd.NaT
    ...
    # ── seccion — Zacatlán: 'seccion'; Atlixco: 'seccion_electoral' ──────────
    if "seccion" in df.columns:
        df["seccion"] = pd.to_numeric(df["seccion"], errors="coerce").astype("Int64")
    df["edad"] = pd.to_numeric(df["edad"], errors="coerce").astype("Int64")

    # ── terminada — si no existe 'estatus', se deja False (transform lo anula)
    if "estatus" in df.columns:
        df["terminada"] = df["estatus"].astype(str).str.strip() == "Terminada"
    else:
        df["terminada"] = False

    # ── municipio — Zacatlán: 'municipio'; Atlixco: 'municipio_texto' ─────────
    if "municipio" not in df.columns and "municipio_texto" in df.columns:
...
def _merge_delta(df_base: pd.DataFrame, df_delta: pd.DataFrame) -> pd.DataFrame:
    ...
    if df_base.empty:
        return df_delta
    # Clave de merge: 'folio' en Zacatlán, 'id_unico' en Atlixco
    key = "folio" if "folio" in df_base.columns else "id_unico"
    folios_delta = set(df_delta[key]) if key in df_delta.columns else set()
    return pd.concat(
        [df_base[~df_base[key].isin(folios_delta)], df_delta],
        ignore_index=True,
    )
